...
if __name__ == "__main__":
	...
	if(sys.argv[1] == "q1"):
		# Write query here
		output = output.groupBy('movieId', 'month', 'year').agg(avg('rating').alias('avg_rating'))
		t.start()
	...
	if(sys.argv[1] == "q2"):
		# Write query here
		output= output.groupBy('user', 'year', 'month').count().filter("count > 10")
		t.start()
	...
	if(sys.argv[1] == "q3"):
		# Write query here
		output = output.groupBy("user", "year", "month").agg(avg('rating').alias('avg_rating')).filter("avg_rating > 5")
		t.start()
	...
	if(sys.argv[1] == "q4"):
		# Write query here
		output= output.groupBy("movieId", "year", "month", "day").count().filter("count > 100")
		t.start()
